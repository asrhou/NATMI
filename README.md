...
Predicting ligand-receptor-mediated interactions in a mouse single-cell RNA-seq dataset using literature supported ligand-receptor pairs and four CPUs:
```bat
   python ExtractEdges.py --species mouse --emFile toy.sc.em.txt --annFile toy.sc.ann.txt --signalType lrc2p --coreNum 4
```

Predicting ligand-receptor-mediated interactions in a human bulk RNA-seq dataset using putative and literature supported ligand-receptor pairs and one CPU:
```bat
...
Detecting changes in edge weight in two output folders (generated by ExtractEdges.py) using literature supported ligand-receptor pairs:
```bat
...
DiffEdges.py cretaes the folder from the names of the two datasets. README.txt (in the output folder) contains information about other files in the folder.
...
Visualising cell-connectivity-summary networks from the results of ExtractEdges.py and DiffEdges.py:
```bat
...
Visualising cell-to-cell communication networks between two cell types using results of ExtractEdges.py and DiffEdges.py:
...
Visualising cell-to-cell communication networks via a ligand-receptor pair from the results of ExtractEdges.py:
