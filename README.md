...
```bat
   python ExtractEdges.py --species human --emFile toy.bulk.em.xls --signalType lrc2a (remove if that is the default or set it to 1 not to 4)
```
...
[Could add --out option and keep the above if 'out' is not specified--a general comment for other outputs too. ]
...
DiffEdges.py creates a folder from the names of the two datasets. README.txt (in the output folder) contains information about other files in the folder.
...
[Might want to put the tested versions here too]
