...
:question:   We found some new human ligand-receptor interactions, is it possible to add them to connectomeDB2020 but keep them in a seperate interaction file when we use NATMI?

:a:: Yes. When you run [ExtractEdges.py](#4.1), in addition to set argument '--interDB' to your interaction file name without extention, please set argument '--interSpecies' to 'expandp' or 'expanda', NATMI will automatically combine your interactions with 'lrc2p' (literature supported ligand-receptor pairs in connectomeDB2020) or 'lrc2a' (literature supported and putative ligand-receptor pairs in connectomeDB2020) database and uses the combined pair list to extract ligand-receptor-mediated edges. Moreover, if '--interSpecies' is set to 'human', NATMI only uses your interactions to build the cell-to-cell communication network.
